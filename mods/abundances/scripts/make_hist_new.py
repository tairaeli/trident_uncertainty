...
##ADD capibilites to do all rays
  pickle_match_off = open(f"matchRay{r}.pickle", 'rb')
  match = pickle.load(pickle_match_off)

  pickle_merge_off = open(f"mergeRay{r}.pickle", 'rb')
  merge = pickle.load(pickle_merge_off)

  pickle_short_off = open(f"shortRay{r}.pickle", 'rb')
  short = pickle.load(pickle_short_off)

  super_clumps = np.load(f'super_clumps_array_Ray{r}.npy')
  ...
  for k in range(len(sup_st)): ##depending on which category each clump belongs to in super_clumps, append its column density to a list
   ...
   ##plot the results##
    ...
    plt.legend()
    plt.title(f"C_IV -- LightRay Index {r} -- Super Clump {k}")
    plt.savefig(f"/mnt/scratch/f0104093/condensed_pipeline_tests/visuals/super_clump_hist/Hist_CIV_RayIndex{r}_SuperClump{k}_.png")
    plt.close()
    print("Plotted!")
