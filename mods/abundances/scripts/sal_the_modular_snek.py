...
stat_dict = abundance_stats(compare_dict, salsa_out_dict, alt_ion_list, raynum)

for ion in alt_ion_list:
    ##categorize clumps##
    datanum = len(pd.read_csv(abun_file, delim_whitespace=True)) ##number of rows on the abundance table
    ndigits= len(str(datanum))
    
    
     ##number of rays used, modify as needed
    ...
    for r in range(raynum):
        ...
        if os.path.exists(f'{stat_path}/Match_{ion}_Ray{r}.pickle'):

            pickle_match_off = open(f"{stat_path}/Match_{ion}_Ray{r}.pickle", 'rb') ##get previously made data
            match = pickle.load(pickle_match_off)
    
            pickle_split_off = open(f"{stat_path}/Split_{ion}_Ray{r}.pickle", 'rb')
            split = pickle.load(pickle_split_off)
    
            pickle_short_off = open(f"{stat_path}/Short_{ion}_Ray{r}.pickle", 'rb')
            short = pickle.load(pickle_short_off)
    
            super_clumps = np.load(f'{stat_path}/super_clumps_array_{ion}_ray{r}.npy')
