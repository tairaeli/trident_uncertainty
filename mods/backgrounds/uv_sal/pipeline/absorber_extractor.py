...
max_impact = float(sal_args["galaxy_settings"]["max_impact"])
min_impact = float(sal_args["galaxy_settings"]["min_impact"])
...
for ion in ion_list:
    ...
    try:
        ...
        abs_ext = salsa.AbsorberExtractor(ds, 
                                          ...
                                          velocity_res =20, 
                                          calc_missing=True,
                                          absorber_min = ion_dat["min_dens (cm^-2)"][ion])
        
        df = salsa.get_absorbers(abs_ext, my_rays, method='spice', fields=other_fields, units_dict=field_units)

        filename = f'{dat_path}/{saved[row_num]}_{ion}.txt'
        
        salsa_out_dict[nion][uvb_name] = df
        ...
        df.to_csv(f'{dat_path}/{saved[row_num]}_{ion}_null.txt')
    
    atom, istate = nion.split(" ")
    ...
    for i,ray_path in enumerate(ray_arr):
        ...
        if os.path.exists(uvb_path+"/ray_dat/"+ion) == False:
            os.mkdir(uvb_path+"/ray_dat/"+ion)
        ...
        save_dens = open(uvb_path+"/ray_dat/"+ion+"/"+f"ray_{i:0{n}d}_dens.pickle","wb")
        pickle.dump(gas_dens, save_dens, protocol=3)
        save_dens.close()
        
        # determining size of SALSA absorbers (kpc)
        ray_absorbers = salsa_out_dict[nion][uvb_name][salsa_out_dict[nion][uvb_name]["lightray_index"]==f'{i:0{n}d}']
        
        for i in ray_absorbers.index:
            abs_size = np.sum(ray_dat.r[("gas","dl")].to("kpc")[ray_absorbers["interval_start"][i]:ray_absorbers["interval_end"][i]])
            
            abs_size_arr.append(abs_size)
            abs_size_cell.append(ray_absorbers["interval_end"][i]-ray_absorbers["interval_start"][i])


# saves the dictonaries so that they can be accesssed later
pickling_match = open(f'{dat_path}/salsa_out_dict.pickle',"wb")
pickle.dump(salsa_out_dict, pickling_match, protocol=3)	
pickling_match.close()

print("DATA SAVED")

# determining median number of SALSA absorbers
for ion in ion_list:
    nion = ion.replace("_"," ")
    num_abs = len(salsa_out_dict[nion][uvb_name]["lightray_index"])
    
    print(ion,uvb_name,"absorber count:",num_abs)

# printing median SALSA absorber size in kpc
print(uvb_name,"median absorber size:",np.median(abs_size_arr),"(kpc)")
print(uvb_name,"median absorber cell size:",np.median(abs_size_cell))
